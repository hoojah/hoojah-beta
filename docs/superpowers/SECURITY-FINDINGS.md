...
| IMG1 | Low | Direct image uploads have **no throttle and no server-side byte cap**. Both direct-upload paths now require a signed-in user — the anonymous-orphan-blob hole is **closed**. But an authenticated user can still mint many/large orphan blobs; the composer's 5 MB check is client-side only. | `image-attachments` branch (Slice 2) | **Anonymous-orphan-blob hole CLOSED.** `POST /uploads/direct` (`DirectUploadsController < ActiveStorage::DirectUploadsController` with `authenticate_user!`) is where the composer posts; the engine's stock **`POST /rails/active_storage/direct_uploads`** (no auth) is now **shadowed** in `config/routes.rb` to the same authenticated controller (app routes take precedence over engine routes), so both paths require auth. `config.active_storage.draw_routes` stays on so the GET proxy/blob routes survive. Covered by `spec/requests/direct_uploads_spec.rb` (unauthenticated stock path rejected, signed-in works). **Still OPEN by design (deferred):** a Rack::Attack throttle on the upload route(s) and a per-blob byte cap enforced at `create` (the stock direct-upload endpoint does not validate `blob_args[:byte_size]`). No throttle exists for these routes today. |
| IMG2 | Medium | **Cross-record blob reuse via a lifted signed_id.** `hujah[image]` accepts any valid blob `signed_id`; a signed_id copied from another user's image URL could be submitted to attach the *same* blob to the attacker's hoojah. Two records then share one blob, and Slice 6's `image.purge_later` on either would destroy the victim's file. | `image-attachments` branch — fixed in Slice 6 | **RESOLVED** (Slice 6, commit `8f362c8`). `Hujah#image_not_reused` validation rejects a blob that already backs a different record's `ActiveStorage::Attachment` (for a new record, any existing attachment of that blob). A legitimate fresh direct-upload has a brand-new, never-attached blob → passes; a lifted blob already carries the victim's attachment row → rejected before save (422, nothing attached). Covered by `spec/requests/compose_spec.rb` ("refuses a signed_id whose blob is already attached to another hoojah"). |
