...
| P5: 7.2→8.0 | 3.3.12 | 8.0.5.1 | 24 | Pinned ~>8.0.0 (~>8.0 would've jumped to 8.1). **App fix:** enum keyword form removed in 8.0 → `enum :category,{…}` / `enum :subject,{…}` in Notification & Flag. load_defaults 8.0. sprockets kept (Propshaft in Phase 8). | (this commit) |
