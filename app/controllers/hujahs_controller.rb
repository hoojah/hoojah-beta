  ...
  def index
    ...
    base = if params[:filter] == "following" && user_signed_in?
      ...
      Hujah.timeline_for(current_user)
        ...
        .includes(image_attachment: :blob, user: {avatar_attachment: :blob}).order(updated_at: :desc)
    else
      # Moderation: E1 sweep — the global/anonymous feed never shows removed claims.
      global = Hujah.not_removed.where(parent_id: nil).includes(image_attachment: :blob, user: {avatar_attachment: :blob}).order(updated_at: :desc)
